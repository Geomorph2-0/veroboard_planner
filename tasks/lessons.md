...
### Never overwrite the user's CLAUDE.md wholesale

- **What:** When `/init` or any skill rewrites CLAUDE.md, check for an existing file first. Replace content (commands, architecture) with the user's original instructions — never replace them
- **Why:** User had a carefully written workflow/principles file that was silently overwritten by the `/init` skill
- **Apply when:** Any time CLAUDE.md is being created or updated
